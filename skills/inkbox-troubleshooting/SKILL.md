...
| Error | Fix |
|---|---|
| `Inkbox is not configured` | Run `inkbox-deepseek setup`, then restart the selected Harness profile. |
...
| `Vault is locked` | This plugin does not expose vault tools. Configure required credentials through the Harness credential store or Inkbox Console. |
...
If `inkbox_whoami` fails, surface the exact error and suggest `inkbox-deepseek doctor` for a local diagnostic pass.
